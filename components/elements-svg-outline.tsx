...
const ElementsSvgOutline = ({
  ...
  return (
    <>
      ...
      >
        <HorizontalLineSVG className="absolute h-[1px] w-[calc(100%_-_22px)] top-0 left-[50%]  -translate-x-[50%]" />
        <HorizontalLineSVG className="absolute h-[1px] w-[calc(100%_-_22px)] bottom-0 left-[50%]  -translate-x-[50%]" />

        <VerticalLineSVG className="absolute w-[1px] h-[calc(100%_-_22px)] left-0 top-[50%]  -translate-y-[50%]" />
        <VerticalLineSVG className="absolute w-[1px] h-[calc(100%_-_22px)] right-0 top-[50%]  -translate-y-[50%]" />
      </div>
      <div
        aria-hidden="true"
        className={cn(
          "absolute inset-[8px] border-bronze border group-hover:scale-101 transition-transform ease-[cubic-bezier(.16,1,.3,1)] duration-500",
          isActive && "drop-shadow-[0_0_4px_rgba(244,234,143,0.5)]",
          className
        )}
      ></div>
