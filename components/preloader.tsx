			...
			<div className="flex md:gap-10 gap-8 flex-col items-center justify-center text-center lg:w-[58.33%] xs:w-[83.33%] w-full mx-auto">
				...
				<div className="flex w-full gap-4 max-md:flex-col items-center justify-center relative px-5">
					<Button
						...
						disabled={
							!isLoaded ||
							(!deviceCapability.isProfiled && !profilingTimedOut) ||
							isExiting
						}
						className="opacity-0 invisible"
						aria-label={
					...
					{isLoaded && (deviceCapability.isProfiled || profilingTimedOut) && (
						<Text
							as="p"
							variant="small"
							ref={loadedTextRef}
							...
							aria-live="polite"
						>
							{profilingTimedOut && !deviceCapability.isProfiled ? (
								<>Ready (basic mode).</>
							) : (
...
const Preloader = ({ children }: PreloaderProps) => {
	...
	const [profilingTimedOut, setProfilingTimedOut] = useState(false);

	// Timeout fallback: if profiling takes too long, allow entry anyway
	// This prevents users from being stuck on very slow devices
	const PROFILING_TIMEOUT_MS = 3000;

	useEffect(() => {
		// If already profiled, no need for timeout
		if (deviceCapability.isProfiled) return;

		const timeoutId = setTimeout(() => {
			if (!deviceCapability.isProfiled) {
				if (process.env.NODE_ENV === "development") {
					console.warn(
						"[Preloader] Device profiling timed out after",
						PROFILING_TIMEOUT_MS,
						"ms - allowing entry with conservative defaults",
					);
				}
				setProfilingTimedOut(true);
			}
		}, PROFILING_TIMEOUT_MS);
	...
	const handleEnterClick = useCallback(() => {
		...
		// Ensure device profiling is complete OR timeout has passed before allowing exit
		const canProceed = deviceCapability.isProfiled || profilingTimedOut;
		if (!canProceed) {
			if (process.env.NODE_ENV === "development") {
