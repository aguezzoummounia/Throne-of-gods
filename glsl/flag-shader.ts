...
export const fragmentShader = `
  ...
  void main() {
      ...
      // 2. Calculate Mouse Distortion (PERFORMANCE TIERED)
      vec2 totalDistortion = vec2(0.0);
      float influence = uMouseInfluence;
      if (uPerformanceLevel > 0.0) { // Skip on 'low'
        vec2 mouseUV = uMouse;
        ...
        totalDistortion = rippleDistortion + fluidDistortion + velocityDistortion + globalWave;
      }
      
      // 3. Apply Distortion and Sample Color (with Chromatic Aberration) (PERFORMANCE TIERED)
      vec2 finalUv = clamp(correctedUv + totalDistortion, 0.0, 1.0);
      vec3 finalColor;
      if (uPerformanceLevel > 1.0 && influence > 0.1) { // Skip on 'low' and 'medium'
      ...
      // 6. Add subtle glow around cursor (PERFORMANCE TIERED)
      if (uPerformanceLevel > 0.0) { // Skip on 'low'
